...
SELECT title, rating from film f 
	...
	AND f.film_id = ALL (SELECT f.film_id FROM film f 
				where f.`length` = (SELECT MIN(`length`) FROM film f2)
...
SELECT first_name, address, (SELECT MIN(p2.amount) FROM payment p2 
								WHERE p2.customer_id = c.customer_id ) as minimo_pago 
